...
pub fn particle_lifetime_normal() -> u32     { f(180) }
/// Critical hit particle lifetime (brighter, stays longer).
pub fn particle_lifetime_crit() -> u32       { f(240) }
/// Healing particle lifetime.
pub fn particle_lifetime_heal() -> u32       { f(160) }
/// Status applied / misc label lifetime.
pub fn particle_lifetime_status() -> u32     { f(140) }
/// Kill reward (+XP +gold) particle lifetime.
pub fn particle_lifetime_reward() -> u32     { f(220) }
/// Spell damage particle lifetime.
pub fn particle_lifetime_spell() -> u32      { f(200) }
/// Backfire particle lifetime.
pub fn particle_lifetime_backfire() -> u32   { f(220) }

/// Upward drift per frame (tile units). Slowed to match longer lifetimes.
pub const PARTICLE_DRIFT: f32 = 0.12;

/// Fraction of lifetime at which fade-out begins (0.0–1.0).
pub const PARTICLE_FADE_START: f32 = 0.65;  // fade over last 35%
