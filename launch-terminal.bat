@echo off
title CHAOS RPG — Terminal (ASCII)
color 0A
...
echo [2/3] Building latest terminal version...
cargo build --release -p chaos-rpg 2>&1 | findstr /C:"Compiling" /C:"Finished" /C:"error"
if %ERRORLEVEL% NEQ 0 (
...
:LAUNCH
echo Launching...
start "" "%REPO_DIR%dist\chaos-rpg.exe"
exit
